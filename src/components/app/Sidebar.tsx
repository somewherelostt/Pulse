  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[220px] bg-pulse-surface border-r border-pulse-border flex flex-col z-40">
      <div className="p-4 border-b border-pulse-border flex items-center gap-2">
        ...
        <span className="font-light text-lg text-pulse-text-primary">Pulse</span>
        <span className="text-[10px] uppercase tracking-wider text-pulse-text-muted bg-pulse-bg px-1.5 py-0.5 rounded">Beta</span>
      </div>
      ...
      <div className="p-3 border-t border-pulse-border space-y-2">
        {!moodLoggedToday && (
          <Link href="/log" className="block">
            <Button size="sm" className="w-full bg-pulse-accent text-pulse-bg hover:bg-pulse-accent/90 relative">
              {moodLoggedToday ? null : <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-pulse-danger animate-pulse" />}
              Log mood today
            </Button>
          </Link>
        )}
        <div className="flex items-center gap-2 px-2 py-1.5">
          <Avatar className="h-8 w-8 bg-pulse-primary/20">
            <AvatarFallback className="text-pulse-primary text-xs">A</AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <p className="text-xs text-pulse-text-secondary truncate">Anonymous user</p>
            <button type="button" className="text-xs text-pulse-text-muted hover:text-pulse-text-secondary flex items-center gap-1">
